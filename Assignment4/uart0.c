...
void task_uart()
{
    static INT8U decode_state = 0;
    static INT8U h1, h2, m1, m2, s1, s2, h, m, s, temp;
    ...
    case 2: // HOUR 2 RECEIVE
    {
        ...
        h = h1+h2;
        put_msg_state(SSM_RTC_HOUR,(h));
        decode_state = 3;
        }
        break;
    }
    case 3: // MINUTE 1 RECEIVE
    {
        if(uart0_rx_rdy())
        {
        m1 = uart0_getc();
    ...
    case 4: // MINUTE 2 RECEIVE
    {
        ...
        m = m1+m2;
        put_msg_state(SSM_RTC_MIN,(m));
        decode_state = 5;
        }
        break;
    }
    case 5: // SECOND 1 RECEIVE
    {
        if (uart0_rx_rdy())
        {
        s1 = uart0_getc();
    ...
    case 8: // HOUR 1 SEND
    {
        if (uart0_tx_rdy())
        {
            temp = get_msg_state(SSM_RTC_HOUR);
            put_msg_state(SSM_RTC_HOUR,temp);

            uart0_putc((temp/10)+'0');
    ...
    case 10: // MINUTE 1 SEND
    {
        if (uart0_tx_rdy())
        {
            temp = get_msg_state(SSM_RTC_MIN);
            put_msg_state(SSM_RTC_MIN,temp);
    ...
    case 12: // SECOND 1 SEND
    {
        if (uart0_tx_rdy())
        {
            temp = get_msg_state(SSM_RTC_SEC);
            put_msg_state(SSM_RTC_SEC,temp);
