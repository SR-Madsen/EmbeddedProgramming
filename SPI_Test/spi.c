...
{
    INT16U rec_data = SSI0_DR_R;
    xQueueSendFromISR(xQueueSPIDATA, void *rec_data, pdTRUE); // <----- TJEK OM PARAMETRE ER RIGTIGE
}
...
{
    static INT8U Tx_state = 0;
    INT16U data;
    INT16U *temp_clear;
    INT8U Tx_counter;

    xQueueReceive(xQueueSPIDATA, void *data, portMAX_DELAY); // <----- TJEK OM PARAMETRE ER RIGTIGE

    if (Tx_counter++ > 19)
    {
        Tx_state = 1;
        Tx_counter = 0;
        xQueueSend(xQueueFILTERDATA, void *data, NULL);
    }
    ...
    case 0:
    {
        GPIO_PORTA_DATA_R |= 0x40; // ADC VÆLGES MED MUX
        ...
        SSI0_DR_R = 0x00; // V_IN VÆLGES MED TREDJE MSB (0 ELLER 1), RESTEN ER LIGEGYLDIGT
        break;
    }
